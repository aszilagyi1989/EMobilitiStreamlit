...
with st.sidebar:
  ...
  for index, row in filtered_Locations.iterrows():
    folium.Marker(
      ...
      popup = f"Üzemeltető: {row['Töltőberendezés üzemeltető neve']}<br>Cím: {(row['IRSZ_VAROS'] + ", " + row['Töltőberendezés közterülete'])}<br>Összteljesítmény (kw): {row['Töltőberendezés által felvehető maximális teljesítmény [kW]']}<br>Csatlakozási pontok: {row['Csatlakozási pontok száma összesen']}"
